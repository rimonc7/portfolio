...
const Projects = () => {
    ...
    return (
        <div id="projects" className={`py-12 transition-all duration-300 ${darkTheme ? "bg-gray-900 text-white" : "bg-gray-200 text-gray-900"}`}>
            <h2 className="text-4xl font-extrabold text-yellow-400 text-center mb-8">Projects</h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 px-6">
                {projects.map((project) => (
                    <div key={project.id} className={`p-6 rounded-lg shadow-lg flex flex-col transition-all duration-300
                        ${darkTheme ? "bg-gray-800" : "bg-white border border-gray-300"}`}>
                        <img src={project.image} alt={project.name} className="rounded-md mb-4" />
                        <h3 className={`text-xl font-semibold mb-2 ${darkTheme ? "text-white" : "text-gray-800"}`}>{project.name}</h3>
                        <p className={`mb-4 flex-grow transition-all duration-300 ${darkTheme ? "text-gray-300" : "text-gray-700"}`}>{project.description}</p>
                        <div className="flex flex-wrap gap-2 mb-4">
                            {project.techStack.map((tech, i) => (
                                <span key={i} className={`text-sm px-3 py-1 rounded-full transition-all duration-300
                                    ${darkTheme ? "bg-gray-700 text-gray-200" : "bg-gray-200 text-gray-700"}`}>
                                    {tech}
                                </span>
                            ))}
                        </div>
                        <div className="flex justify-center w-full">
                            <Link to={`/projects/${project.id}`} className={`px-4 py-2 rounded-md flex-1 text-center transition-all duration-300
                                ${darkTheme ? "bg-blue-500 text-white" : "bg-blue-600 text-white hover:bg-blue-700"}`}>
                                View More / Details
                            </Link>
                        </div>
                    </div>
